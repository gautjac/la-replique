...
import { atelier, type DramaturgieRes, type RelanceRes } from "../api";
...
type Tool = "relance" | "dramaturgie" | "traduire";
...
export function Atelier({ play, commit, onCreatePlay, onToast }: AtelierProps) {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-1.5 rounded-xl bg-desk p-1 ring-1 ring-desk-rule">
        {(["relance", "dramaturgie", "traduire"] as Tool[]).map((tl) => (
          <button
            key={tl}
            onClick={() => setTool(tl)}
          ...
          >
            {t(tl === "relance" ? "aiRelance" : tl === "dramaturgie" ? "aiDramaturgie" : "aiTraduire")}
          </button>
        ))}
      </div>
      ...
      {tool === "traduire" && <TraduireTool play={play} onCreatePlay={onCreatePlay} onToast={onToast} />}
...
// ————————————————————————————————————————————————————————————————
function TraduireTool({ play, onCreatePlay, onToast }: { play: Play; onCreatePlay: (p: Play) => void; onToast: (m: string) => void }) {
  const { t, locale } = useUI();
  const r = useRunner();
    ...
    <div className="space-y-3">
      ...
      {preview && (
        <div className="rise space-y-3 rounded-xl bg-desk p-4 ring-1 ring-gel/40">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={create} className="rounded-lg bg-gel px-3.5 py-2 text-sm font-semibold text-white hover:bg-gel-bright">
              {t("aiApplyTranslation")}
            </button>
            <button onClick={run} className="rounded-lg bg-desk-light px-3.5 py-2 text-sm text-white ring-1 ring-desk-rule hover:bg-desk-rule">
              {t("aiRegenerate")}
            </button>
            <button onClick={() => setPreview(null)} className="rounded-lg px-3 py-2 text-sm text-ink-faint hover:text-white">
              {t("aiDismiss")}
            </button>
          </div>
          <p className="text-[11px] text-ink-faint">
            {locale === "fr"
              ? "Crée une nouvelle pièce ; ton original n'est pas touché."
              : "Creates a new play; your original is untouched."}
          </p>
        </div>
      )}
    </div>
